...
def first_setup(standalone):
    ...
    if click.confirm("Do you want to set hardware informations?"):
        ...
        gb_count = click.prompt("Enter the number of grainbin reader chips on the board", default=0)
        set_hardware_info(hardware_version, gb_count)

    if click.confirm("Do you want to set the internal and external sensor information?"):
        ...
        for sensor, temperature in current_sensors.items():
            click.echo("{0}. Sensor: {1} Temperature: {2}".format(x, sensor, temperature))
            x = x + 1

        interior_sensor = click.prompt("Select which sensor is the internal temperature", default=1)
        exterior_sensor = click.prompt("Select which sensor is the external temperature", default=2)
...
def get_interface_details(interface):
    ...
    info = {}
    info['name'] = interface
    info['is_for_fm'] = click.confirm("Is this interface for Farm Monitor(y) or for external access(n)", default=False)
    info['state'] = click.prompt("Should this interface be configured as an 'ap' or 'dhcp'", default='dhcp')
    if info['state'] == 'ap':
        creds = {}
...
def initialize_grainbin(device_id, grainbin_reader_count):
    ...
    device_id is the id of the device. Duh!
    grainbin_reader_count is how many 1W chips the device has
    """
