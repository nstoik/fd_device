...
def hostapd_file(interface, wifi_ssid, wifi_password):
    ...
    contents = ('# This is the name of the WiFi interface\n'
                'interface={interface}\n'
                '\n'
...
def interface_file(wlan0_dhcp=True, wlan1_dhcp=True):
    ...
    wlan_dhcp = ('allow-hotplug {interface}\n'
    ...
    contents = ('# Include files from /etc/network/interfaces.d:\n'
...
def iptables_file(external_interface, internal_interface, flush_only=False):
    ...
    if not flush_only:
        command = ['sudo', 'iptables', '-t', 'nat', '-A', 'POSTROUTING',
                   '-o', external_interface, '-j', 'MASQUERADE']
        subprocess.check_call(command)

        command = ['sudo', 'iptables', '-A', 'FORWARD', '-i', external_interface,
                   '-o', internal_interface, '-m', 'state', '--state', 'RELATED,ESTABLISHED',
                   '-j', 'ACCEPT']
        subprocess.check_call(command)

        command = ['sudo', 'iptables', '-A', 'FORWARD', '-i', internal_interface,
...
def dnsmasq_file(interface):
    ...
    contents = ('interface={interface}           # Use interface {interface}\n'
                ...
                'dhcp-range=10.10.1.50,10.10.1.150,12h').format(interface=interface)
...
def dhcpcd_file(interface=None):
    ...
    top_part = ('# Inform the DHCP server of our hostname for DDNS.\n'
                'hostname\n'
                '\n'
...
def wpa_supplicant_file(networks):
    ...
    for network in networks:
        contents.append(network_ssid.format(ssid=network['ssid'], password=network['password']))

    # compile all the contents into one string and write it to file
    with open('/tmp/wpa_supplicant_temp', 'w') as f:
        f.write("".join(contents))

    command = ['sudo', 'mv', '/tmp/wpa_supplicant_temp', '/etc/wpa_supplicant/wpa_supplicant.conf']
    subprocess.check_call(command)
